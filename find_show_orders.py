...
dancer_wait_times = {
    ...
    if dancer_routine_counts[dancer]
    >= 2  # No need to track wait times for dancers who are only in 1 routine.
}
...
slot_routine_count = [
    sum(routine_slot_flags[routine][i] for routine in routines.keys()) for i in range(num_slots)
]
            ...
            if other_routine != routine:
                for i in range(num_slots):
                    model.add_implication(
                        routine_slot_flags[routine][i], ~routine_slot_flags[other_routine][i]
                    )
